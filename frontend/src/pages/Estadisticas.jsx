  ...
  return <section className="estadisticas">
    ...
    {error ? <p role="alert">{error}</p> : !resumen ? <p role="status">Cargando estadísticas...</p> : invalido ? <p role="alert">La fecha Desde debe ser anterior o igual a Hasta.</p> : <>
      ...
      <div className="estadisticas-graficos">
        ...
        <article className="estadisticas-panel"><h2>Inspección final</h2><p>Estado actual de los pares producidos en el período.</p><div className="estadisticas-inspeccion">{["Conforme", "No conforme", "Pendiente"].map((estado, i) => <div key={estado}><span className={`estadisticas-estado estado-${i}`}>{estado}</span><strong>{numero(paresEstado(estado))} pares</strong><small>{resumen.total ? Math.round(paresEstado(estado) / resumen.total * 100) : 0}% del total</small></div>)}</div></article>
