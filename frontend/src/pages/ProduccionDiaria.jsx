...
export default function ProduccionDiaria() {
  ...
  }), [historialFiltrado, ordenHistorial, direccionHistorial, grupoHistorial]);
    ...
    const datos = {
      ...
      bloques: bloques.map((bloque) => ({
        ...
        lineas: bloque.lineas.map((linea) => ({
          ...
          lote_puntera_id: Number(linea.lote_puntera_id) || null,
          lote_pu_id: Number(linea.lote_pu_id) || null,
          ...
          materiales_extra: linea.materiales_extra.filter((material) => material.lote_id).map((material) => ({ lote_id: Number(material.lote_id) })),
          estado_inspeccion: linea.estado_inspeccion,
          pares_defectuosos: linea.estado_inspeccion === "No conforme" ? Number(linea.pares_defectuosos) : 0,
      ...
      {formularioAbierto && <form className="produccion-diaria-form" onSubmit={guardar}>
        ...
        {bloques.map((bloque, indiceBloque) => <div className="ui-form-card produccion-bloque" key={indiceBloque}>
          ...
          <div className="produccion-inyectora-grid">
            <label>Inyectora<div className="produccion-selector-inyectora"><Selector value={bloque.maquinas_id_maquina} onChange={(e) => actualizarBloque(indiceBloque, "maquinas_id_maquina", e.target.value)} required><option value="">Seleccione inyectora</option>{maquinas.map((maquina) => <option key={maquina.id_maquina} value={maquina.id_maquina}>{maquina.nombre_maquina || maquina.maquina}</option>)}</Selector><button type="button" onClick={() => setAltaMaquinaBloque(indiceBloque)} title="Crear inyectora" aria-label="Crear inyectora">+</button></div></label>
            <div className="produccion-operarios"><span>Operarios de inyección</span>{bloque.operarios_inyeccion.map((nombre, indice) => <div key={indice}><NombreSugerido value={nombre} onChange={(e) => actualizarOperarioInyeccion(indiceBloque, indice, e.target.value)} required />{bloque.operarios_inyeccion.length > 1 && <button type="button" onClick={() => quitarOperarioInyeccion(indiceBloque, indice)} aria-label="Quitar operario">×</button>}</div>)}<button type="button" className="produccion-agregar-operario" onClick={() => agregarOperarioInyeccion(indiceBloque)}>+ Agregar operario</button></div>
          </div>
          ...
          {bloque.lineas.map((linea, indiceLinea) => <div className="produccion-linea" key={indiceLinea}>
            ...
            <fieldset className="produccion-inspeccion"><legend>Inspección final</legend><div className="produccion-inspeccion-opciones">{["Pendiente", "Conforme", "No conforme"].map((estado) => <label key={estado} className={`inspeccion-opcion inspeccion-opcion-${estado.toLowerCase().replace(" ", "-")}`}><input type="radio" name={`inspeccion-${indiceBloque}-${indiceLinea}`} value={estado} checked={linea.estado_inspeccion === estado} onChange={(e) => actualizarLinea(indiceBloque, indiceLinea, "estado_inspeccion", e.target.value)} /><span>{estado === "Pendiente" ? "Pendiente de inspección" : estado}</span></label>)}</div>{linea.estado_inspeccion === "No conforme" && <ParesDefectuosos value={linea.pares_defectuosos} total={Object.values(linea.talles).reduce((sum, cantidad) => sum + Number(cantidad || 0), 0)} onChange={(value) => actualizarLinea(indiceBloque, indiceLinea, "pares_defectuosos", value)} />}{linea.estado_inspeccion === "No conforme" && <label className="produccion-observacion">Observación de la no conformidad<textarea value={linea.observacion_inspeccion} onChange={(e) => actualizarLinea(indiceBloque, indiceLinea, "observacion_inspeccion", e.target.value)} placeholder="Describí el defecto o motivo del rechazo" required rows="2" /></label>}</fieldset>
          </div>)}
