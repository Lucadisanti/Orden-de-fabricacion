  ...
  return <section className="produccion-diaria">
    ...
    {cargando ? <p>Cargando datos…</p> : <>
      ...
      <div className={`produccion-historial${formularioAbierto ? " produccion-historial-separado" : ""}`}>
        ...
        <div className="ui-table-card"><table className="ui-data-table produccion-historial-tabla"><colgroup>{[9, 11, 13, 9, 21, 13, 13, 11].map((ancho, indice) => <col key={indice} style={{ width: `${ancho}%` }} />)}</colgroup><thead><tr><th>Orden</th><th>Artículo</th><th>Producto</th><th>Color</th><th>Inyectora</th><th>Fecha</th><th>Inspección</th><th>Total de pares</th></tr></thead><tbody>{historialVisible.length ? historialVisible.map((item, indice) => {
