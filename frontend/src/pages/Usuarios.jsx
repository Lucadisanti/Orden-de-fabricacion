...
export default function Usuarios() {
  ...
  const cargar = () => axios.get("/api/auth/usuarios").then((response) => setUsuarios(response.data));
  useEffect(() => { cargar(); }, []);
  ...
  const crear = async (event) => {
    ...
    try {
      ...
      setMensaje(err.response?.data?.error || "No se pudo crear.");
    }
  };
  ...
  const cambiarClave = async (event) => {
    ...
    try {
      ...
      setMensaje(err.response?.data?.error || "No se pudo actualizar.");
    }
  };
  ...
  const eliminar = async () => {
    try {
      ...
      setMensaje(err.response?.data?.error || "No se pudo eliminar el usuario.");
    }
  };
  ...
  const generarCodigo = async (id) => {
    try {
      ...
      setMensaje(err.response?.data?.error || "No se pudo generar el código.");
    }
  };
  ...
  return <section>
    <div className="ui-page-header ui-page-header-row">
      ...
      {!mostrar && <button className="ui-btn ui-btn-primary" onClick={() => setMostrar(true)}>+ Nuevo usuario</button>}
    </div>
    {mostrar && <div className="ui-form-card"><h2>Nuevo usuario</h2>
      <form className="form-planilla" onSubmit={crear}>
        ...
        <div className="ui-form-actions"><button className="ui-btn ui-btn-primary">Crear usuario</button><button type="button" className="ui-btn ui-btn-secondary" onClick={() => setMostrar(false)}>Cancelar</button></div>
      </form>
    </div>}
    ...
    <div className="ui-table-card"><table className="ui-data-table"><thead><tr><th>Usuario</th><th>Nombre</th><th>Rol</th><th>Estado</th>{esMaestro && <th>Recuperación</th>}<th>Acciones</th></tr></thead><tbody>
      {usuarios.map((usuario) => <tr key={usuario.id_usuario}>
        ...
        <td><div className="user-actions"><button className="ui-btn ui-btn-secondary" onClick={() => abrirEdicion(usuario)}>Editar usuario</button><button className="ui-btn ui-btn-secondary" onClick={() => abrirCambioClave(usuario)}>Cambiar contraseña</button>{esMaestro && usuario.rol === "maestro" && usuario.activo && <button className="ui-btn ui-btn-secondary" onClick={() => generarCodigo(usuario.id_usuario)}>Generar código</button>}{puedeEliminar(usuario) && <button className="ui-btn ui-btn-danger" onClick={() => setConfirmarEliminacion(usuario)}>Eliminar</button>}</div></td>
      </tr>)}
    </tbody></table></div>
    {cambioClave && <div className="usuarios-modal-backdrop" role="presentation">
      <form className="usuarios-modal" onSubmit={cambiarClave} role="dialog" aria-modal="true" aria-labelledby="cambiar-clave-titulo">
        ...
        <div className="ui-form-actions"><button className="ui-btn ui-btn-primary">Guardar contraseña</button><button type="button" className="ui-btn ui-btn-secondary" onClick={() => setCambioClave(null)}>Cancelar</button></div>
      </form>
    </div>}
    {edicion && <div className="usuarios-modal-backdrop" role="presentation">
      <form className="usuarios-modal" onSubmit={guardarEdicion} role="dialog" aria-modal="true" aria-labelledby="editar-usuario-titulo">
        ...
        <div className="ui-form-actions"><button className="ui-btn ui-btn-primary">Guardar cambios</button><button type="button" className="ui-btn ui-btn-secondary" onClick={() => setEdicion(null)}>Cancelar</button></div>
      </form>
    </div>}
    {confirmarEliminacion && <div className="usuarios-modal-backdrop" role="presentation">
      <div className="usuarios-modal usuarios-modal-confirmar" role="dialog" aria-modal="true" aria-labelledby="eliminar-usuario-titulo">
        ...
        <div className="ui-form-actions"><button className="ui-btn ui-btn-danger" onClick={eliminar}>Eliminar usuario</button><button type="button" className="ui-btn ui-btn-secondary" onClick={() => setConfirmarEliminacion(null)}>Cancelar</button></div>
      </div>
    </div>}
  </section>;
}
